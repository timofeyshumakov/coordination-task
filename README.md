# Согласование счетов — приложение Bitrix24

Веб-приложение для портала Bitrix24: список элементов смарт-процесса **«Согласование счетов»** (ID **1126**) на стадии «На согласовании», согласование и отклонение с переводом на нужные стадии CRM.
...
Целевой смарт-процесс: **entityTypeId = 1126**, название на портале — **«Согласование счетов»**.
...
| Отклонить | Красная кнопка → диалог с необязательным комментарием → стадия `DT1126_144:UC_CB3Y3K` |
| Согласовать | Зелёная кнопка → стадия `DT1126_144:CLIENT` без диалога |
...
- Node.js 18+
- Портал Bitrix24 со смарт-процессом **«Согласование счетов»** (ID **1126**)
- Подключённые scope приложения (см. [Требуемые модули](#требуемые-модули))
...
| Параметр | Значение |
|----------|----------|
| Название | Согласование счетов |
| Тип сущности (`entityTypeId`) | `1126` |
| Рабочая стадия (список) | `DT1126_144:UC_2M5QYG` |
| Стадия после отклонения | `DT1126_144:UC_CB3Y3K` |
| Стадия после согласования | `DT1126_144:CLIENT` |
...
При регистрации локального приложения в Bitrix24 необходимо указать scope (права):

| Модуль | Назначение в приложении |
|--------|-------------------------|
| **crm** | `crm.item.list`, `crm.item.fields`, `crm.item.update` — элементы смарт-процесса «Согласование счетов» |
| **user** | `user.get` — ФИО ответственного |
| **department** | `department.get` — название отдела по `UF_DEPARTMENT` ответственного |

Без этих модулей загрузка таблицы или действия «Согласовать» / «Отклонить» завершатся ошибкой доступа.
